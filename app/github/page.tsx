...
export default function GithubPage() {
  const primaryRepos = [
    ...
    },
    {
      ...
      desc: 'MCP server that enables Claude Desktop to perform Person CRUD operations via natural language. Connects seamlessly to the person-search backend.',
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-10">
      ...
      {/* Primary repos highlight */}
      <div className="space-y-4 mb-8">
        {primaryRepos.map((repo) => (
          <div key={repo.name} className="p-6 rounded-2xl bg-accent/5 border border-accent/30">
            <div className="flex items-start justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                <div>
                  <h2 className="font-display font-semibold text-lg">{repo.name}</h2>
                </div>
              </div>
              <a
                href={repo.url}
      ...
      <div className="p-6 rounded-2xl bg-card-surface border border-custom">
        ...
        <div className="space-y-3">
          {[
            ...
            { step: '5', cmd: 'npx prisma migrate dev && npx prisma db seed', desc: 'Set up database and seed data' },
            { step: '6', cmd: 'npm run dev', desc: 'Start the development server at localhost:3000' },
          ].map(({ step, cmd, desc }) => (
